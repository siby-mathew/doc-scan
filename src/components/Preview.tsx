  ...
  const handleSend = async () => {
    ...
    try {
      ...
      for (const base64 of images) {
        const correctedBase64 = await correctImageOrientation(base64);
        ...
        let embed;
        if (correctedBase64.startsWith("data:image/png")) {
...
function getExifOrientation(base64: string): number {
  ...
  while (offset < view.byteLength) {
    if (view.getUint16(offset) === 0xffe1) {
      ...
      if (
        view.getUint32(marker) === 0x45786966 &&
        view.getUint16(marker + 6) === 0x4949
      ) {
        const orientationOffset = marker + 10;
        const orientation = view.getUint16(orientationOffset);
        return orientation;
      }
    }
    offset += 2;
...
// Apply Orientation Fix Using Canvas
